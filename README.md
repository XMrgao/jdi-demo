...
​			不想修改任何代码，但是想监控变量的值变化事件。



先创建一个测试代码,模拟场景，代码很简单就不解释了
...
```
-agentlib:jdwp=transport=dt_socket,address=ip地址:端口号,suspend=n,server=y
```

然后是调试程序,注意调试程序，依赖jdk自带的tools.jar 包，一般在jdk安装根目录下的lib目录里，自行添加依赖，不多说
...
运行调试程序，控制台就会打印变化后的值和调用堆栈：

```
[JDI: EventSet: SUSPEND_EVENT_THREAD]
[JDI: Event: ModificationWatchpoint@Main$CCC:26 in thread main]
Main.value发生变化：12
stack trace in thread "main"
	at Main.main(java.lang.String[]) -> (Main.java from InputFileObject:7)
	at Main$AAA.run() -> (Main.java from InputFileObject:14)
	at Main$BBB.run() -> (Main.java from InputFileObject:20)
	at Main$CCC.run() -> (Main.java from InputFileObject:26)
```
